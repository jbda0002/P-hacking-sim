...
Adding a dependent variable with a correlation of (\textit{r}=0.5) to the one used in the base line model and the average between the two dependent variables increased the FPP for all model sets. The increase in FPP is mainly driven by the increase in the models in each set as there is three times as many models. There was no general increase for the FPR.  
