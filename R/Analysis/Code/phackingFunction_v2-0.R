...
phackingFunction<-function(data,y,H_1,Power_2 = TRUE,Power_3=FALSE,Power_12=FALSE
                           ,Power_23=FALSE,Power_123=FALSE
                           ,SD=FALSE,Per=FALSE,Main=TRUE){
  ## Loading liberaries
  library(data.table)
  library(dplyr)
  ...
  if(Power_12==TRUE & Main == TRUE){
    ...
    Power12=FoirmulasInC
      ...
      ## Combine Power(2) with Power(1)
      Formulas=c(Power1,FormulasIN)
  ...
  if(Power_3==TRUE & Main==TRUE){
    ...
    Power3=sapply(CombinTwo_2,function(i)
      ...
      for (j in 1:length(Test)) {
        ## Test if variable is part of the interaction term
        loop=grep(Test[j], Power3,invert=TRUE)
        
        ## For those where the term is not part of the interaction term, add them
        for (k in loop){
          
          ## Paste on the main effects that are needed
          Form= paste(Power3[k],paste("+",Test[j]))
          ## Change the | to a +
          Form=gsub("\\|", "+", Form)
          ## Put them into a list
          FoirmulasInC=rbind(Form,FoirmulasInC)
      ...
      Power13=FoirmulasInC
    
    
    if(Power_2==TRUE & Power_12==FALSE){
      Formulas=c(Power1,Power2,Power3,Power13)
      }
    if(Power_2==TRUE & Power_12==TRUE){
        Formulas=c(Power1,Power2,Power12,Power3,Power13)
      }
    else{
      Formulas=c(Power1,Power3,Power13)
    }
  }
  
  if(Power_3==TRUE & Main==FALSE){
    ## Make Power(3) to put together with Power(1) and Power(2)
    ...
    Power13=apply(FormulasIN13, 1, paste, collapse="+") 
  ...
  if(Power_123==TRUE & Main==TRUE){
    ...
    FormulasIN=apply(Power23, 1, paste, collapse="+")
    ...
    ## Combine Power(1+2) with Power(1) and Power(2)
    if(Power_12==TRUE){
      Formulas=c(Power1,Power2,Power3,Power13,FormulasIN,FoirmulasInC,Power12)
    }
    else{
      Formulas=c(Power1,Power2,Power3,FormulasIN,FoirmulasInC)
    }
   
    Power123=c(FormulasIN,FoirmulasInC)
    
  }
  if(Power_123==TRUE & Main==FALSE){
    ...
    Formulas=c(apply(Power123, 1, paste, collapse="+"),Power1,Power2,Power3,Power12,Power13,Power23)
      ...
      Formulas=c(apply(Power123, 1, paste, collapse="+"),Power1,Power2,Power3,Power13,Power23)
  ...
  for(i in 1:length(models)){
    ...
    if(Main==FALSE){
      if(grepl("x1:",mod, fixed=TRUE)==TRUE){
        coef<-as.data.frame(coef)
        coef$names<-rownames(coef)
        coef<-as.data.table(coef)
